package smartfish.robotlegs.extensions.debug.impl
{
    ...
    public class DLogViewer extends Sprite
    {
        ...
        public function DLogViewer(console:DConsole):void
        {
			_console = console;
            layout();
            addListeners();
			...
			name = "Console";
			_console.registerCommand("copy", onBitmapDoubleClick, "复制日志内容到粘贴板.");
			_console.registerCommand("clear", onClearCommand, "清除日志.");
        }
        ...
        protected function onInputKeyDown(event:KeyboardEvent):void
        {
            ...
            else if(event.keyCode == Keyboard.TAB)
            {
                ...
                var list:Array = _console.getCommandList();
            ...
            else if(event.keyCode == _console.hotKeyCode)
            {
                // Hide the console window, have to check here due to 
                // propagation stop at end of function.
               parent.removeChild(this);
				/*Tweener.removeTweens(this);
				Tweener.addTween(this,{time:.4,transition:Equations.easeOutExpo,y:-this.height,onComplete:function ():void{
					parent.removeChild(this);
				}});*/
                deactivate();
            }
        ...
        {
            addLogMessage("CMD", ">", _input.text);
			_console.processLine(_input.text);
        ...
        public function addLogMessage(level:String, loggerName:String, message:String):void
        {
            ...
            {
                var text:String = ((_console.verbosity > 0) ? level + ": " : "") + loggerName + " - " + msg;
